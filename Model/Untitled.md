...
- **更新过程**:
  ...
  * **步骤 2**: 使用**目标网络**计算所有 `s'` 的 `max Q` 值，得到 `y_target`
  ...
  ```python
  # 输入当前状态，得到真实Q值 [b,4]==>[b,2]==>[b,1]
  q_values = self.q_net(states).gather(1, actions)  # [b,1]
  # 输入下一时刻的状态[b,4]-->目标网络输出下一时刻对应的动作q值[b,2]-->
  # 选出下个状态采取的动作中最大的q值[b]-->维度调整[b,1]
  max_next_q_values = self.target_q_net(next_states).max(1)[0].view(-1, 1)
  # 下一时刻的目标Q值：即时奖励+折扣因子*下个时刻的最大回报
...
**核心思想:**标准 DQN 在计算 `y_target` 时，**选择和评估**下一个状态 $s'$ 的最优动作都依赖于目标网络 `Q_target`。这会导致 Q 值被**显著高估 (overestimation)**，因为 `max` 操作会偏向于带有正误差的估计值，Double DQN 解决了这个问题。它将**动作选择**和**价值评估**解耦：
...
- **代码修改**:
  ...
  ```python
  # 1. 使用当前在线网络选择s‘下最优动作a*的索引 [b, n_actions] -> [b]
  max_actions = self.q_net(next_states).max(1)[1] # 得到的是索引，不是Q值
  # 2. 使用目标网络评估这个动作a*的Q值，用gather按索引取值
  next_q_values = self.target_q_net(next_states)
  max_next_q_values = next_q_values.gather(1, max_actions.unsqueeze(1)).squeeze(1) # [b]
  # 3. 计算Double DQN的y_target
  q_targets = rewards + self.gamma * max_next_q_values.view(-1, 1) * (1 - dones)
  ```
